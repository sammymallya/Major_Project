...
# ─────────────────────────────────────────────────────────
def pipeline_fn(query: str, mode: str) -> dict:
    """
    Wrapper that connects evaluator to orchestration pipeline.
    Returns dict with separate kg_results and vector_snippet
    so evaluator can measure KG and Vector metrics separately.
    ...
    try:
        ...
        return {"kg_results": [], "vector_snippet": None}

    kg_results     = []
    vector_snippet = None

    # ── KG results — re-run to get full structured dicts ──
...
def run_interactive(formatter: Formatter):
    ...
    while True:
        try:
            ...
            raw_output = pipeline_fn(user_input, mode)
            if raw_output.get("kg_results") or raw_output.get("vector_snippet"):
                metrics = evaluator.evaluate(
                    query=user_input,
                    kg_results=raw_output.get("kg_results", []),
                    vector_snippet=raw_output.get("vector_snippet"),
                    mode=mode
                )
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        raw_output = pipeline_fn(query, mode)
        metrics = evaluator.evaluate(
            query=query,
            kg_results=raw_output.get("kg_results", []),
            vector_snippet=raw_output.get("vector_snippet"),
