...
reranker, KG (stub), builds a prompt via stub Prompt Generator, and calls
...
def run_pipeline(query: str, test_mode: TestMode) -> QueryResponse:
    ...
    if test_mode == "none":
        ...
        # KG path (stub returns empty list)
        if structured.cypher_query:
            kg_triples = fetch_kg(structured.cypher_query)
            logger.info("KG path returned %d triple(s) (stub)", len(kg_triples))
        else:
    ...
    if not context_used.vector_snippet and context_used.kg_triples_count is None:
        context_used = None
